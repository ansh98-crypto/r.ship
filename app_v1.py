...
_has_anthropic_pkg = _anthropic is not None
_has_anthropic_key = bool(ANTHROPIC_KEY)
USE_OPENAI = bool(OPENAI_KEY)

# Claude is only used if: package installed + key present + not explicitly forced to openai
USE_CLAUDE = (
    _has_anthropic_pkg and _has_anthropic_key
    and _forced != "openai"
    and (_forced == "claude" or True)  # default: prefer Claude when available
)
# If forced to openai explicitly, always use OpenAI
if _forced == "openai":
    USE_CLAUDE = False
...
_backend = f"Claude ({HAIKU})" if USE_CLAUDE else f"OpenAI ({OAI_CHAT})"
print(f"AI backend : {_backend}")
print(f"Embeddings : OpenAI {OAI_EMBED}")
if not USE_CLAUDE:
    print(f"Note: Claude disabled — {'LLM_BACKEND=openai forced' if _forced=='openai' else 'no Anthropic key/package'}. Using OpenAI for chat.")

# ── Init clients ───────────────────────────────────────────────────────────
claude_client = _anthropic.Anthropic(api_key=ANTHROPIC_KEY) if USE_CLAUDE and _anthropic else None
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app_v1:app", host="127.0.0.1", port=8000, reload=True)
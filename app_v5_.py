...
def delete_log(lid: str):
    with db() as c:
        ...
        # Get linked tasks first
        tasks = [r2d(r) for r in c.execute(
            "SELECT * FROM tasks WHERE source_log_id=?",
            (lid,)
        )]
        ...
        for task in tasks:
            ...
            if href and CALENDAR_OK:
                try:
                    calendar_tools.delete_event(href)
                except Exception as e:
                    print("Calendar delete failed:", e)

        # Delete linked tasks
        c.execute("DELETE FROM tasks WHERE source_log_id=?", (lid,))

        # Delete theme mappings
        c.execute("DELETE FROM log_themes WHERE log_id=?", (lid,))

        # Delete log
        deleted = c.execute("DELETE FROM logs WHERE id=?", (lid,)).rowcount
...
def run_agent(data: AgentMessage):
    ...
    try:
        ...
        print("AGENT INVOKE ERROR:", repr(e), flush=True)
        return {
            "response": "I hit an agent parsing error before completing the action. Check terminal logs for AGENT INVOKE ERROR.",
            "actions_taken": 0
        }
    ...
    content = final_msg.get("content", "").strip() if final_msg else ""

    try:
        payload = json.loads(content) if content else {"final": "(no response from agent)"}
    except Exception:
        payload = {"final": content or "Agent returned an empty/non-JSON response."}

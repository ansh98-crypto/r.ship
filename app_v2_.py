...
themes clearly implied by the log content.""")
        ...
        print(f"ai_enhance_log parse error: {e}\nRaw: {raw!r}")
...
If no time-sensitive signals found, return {{"triggers": []}}""", max_tokens=600)
        ...
        print(f"ai_scan_for_alerts parse error: {e}\nRaw: {raw!r}")
        return []
...
{{
  ...
  "date_range_start": "YYYY-MM-DD if query implies a date window (e.g. '3rd week of april' → '2026-04-15'), else null",
  "date_range_end":   "YYYY-MM-DD if query implies a date window (e.g. '3rd week of april' → '2026-04-21'), else null"
}}""")
        return _parse_json(raw)
    except Exception as e:
        print(f"ai_parse_intent parse error: {e}\nRaw: {raw!r}")
        return {"intent": query, "enriched_query": query, "is_nudge_request": False, "name_hint": None, "topic_hint": None, "date_range_start": None, "date_range_end": None}
...
def search(data: SearchQ):
    ...
    # Parse date window from intent (may be None)
    dr_start = intent.get("date_range_start")
    dr_end   = intent.get("date_range_end")
    try:
        dt_start = datetime.strptime(dr_start, "%Y-%m-%d") if dr_start else None
        dt_end   = datetime.strptime(dr_end,   "%Y-%m-%d") if dr_end   else None
    except Exception:
    ...
    for contact in contacts:
        ...
        for log in clogs:
            ...
            # Date-range boost: if query has a date window, boost logs whose text
            # contains a date that falls within that window, and penalise those clearly outside
            if dt_start and dt_end:
                # Extract any dates mentioned in the log text
                log_text = log.get("text", "")
                found_dates = []
                for fmt in ["%d %B %Y", "%d %b %Y", "%B %d %Y", "%b %d %Y",
                             "%d/%m/%Y", "%Y-%m-%d"]:
                    # Try attaching current year to short dates like "18th april"
                    for attempt in [log_text, log_text + f" {datetime.now().year}"]:
                        try:
                            parsed = dtparse.parse(attempt, fuzzy=True, default=datetime(datetime.now().year, 1, 1))
                            found_dates.append(parsed)
                            break
                        except Exception:
                            pass
                    break  # only need one successful parse per log

                if found_dates:
                    log_dt = found_dates[0]
                    if dt_start <= log_dt <= dt_end:
                        score += 0.35   # strong boost: date is inside the window
                    elif abs((log_dt - dt_start).days) <= 7 or abs((log_dt - dt_end).days) <= 7:
                        score += 0.10   # mild boost: close to window
                    else:
                        score -= 0.15   # penalise: date clearly outside window
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app_v2_:app", host="127.0.0.1", port=8000, reload=True)